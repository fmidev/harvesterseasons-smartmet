...
def custom_objective(y_pred, dtrain):
    ...
    lambda_val = 0.1  # Ensure lambda_val is defined
...
def custom_eval_metric(y_pred, dtrain):
    ...
    # Calculate MSE and variance penalty
...
model.save_model(mod_dir + mod_name)

# Plot predictions vs actual values
plt.figure(figsize=(10, 6))
plt.plot(test_set['valid_time'], test_predictand, label='True Values', color='blue')
plt.plot(test_set['valid_time'], predictand_pred, label='Predicted Values', color='red', linestyle='--')
plt.xlabel('Time')
plt.ylabel(f'{predictand} Value')
plt.title(f'{predictand} Prediction vs Actuals')
plt.legend()
plt.show()
