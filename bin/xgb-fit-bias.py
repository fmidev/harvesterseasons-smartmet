...
for y in years:
    ...
    if y == 2022: 
        ds = ds.isel(time=slice(0, -1)) # 2022 has one extra timestep compared to sfc which messed up merging
...
print(example_filter[['lat', 'lon','ERA5_2t','t850','t925','2t']])
...
preds=['lat', 'lon', 'ERA5_anor', 'ERA5_z', 'ERA5_lsm','ERA5_slor', 'ERA5_sdor', 'month', 'lrate',
       #'DTM_height',# 'DTM_slope', 'DTM_aspect',
       ]+ecsf_sl_vars+[f"{var}850" for var in ecsf_pl_vars] + [f"{var}925" for var in ecsf_pl_vars]
