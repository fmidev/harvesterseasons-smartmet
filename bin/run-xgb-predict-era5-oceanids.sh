...
[ -f ens/ec-${bsf}_$year${month}_pl-pp-unbound-24h-$abr-50-fix.grib ] && ! [ -f ens/ec-${bsf}_${year}${month}_pl850-pp-${harbor}-50.grib ] && \
seq 0 50 | parallel cdo --eccodes -O -b P8 sellonlatbox,$bbox -selname,z,q,t,u,v,kx -sellevel,85000 ens/ec-${bsf}_$year${month}_pl-pp-unbound-24h-$abr-{}-fix.grib ens/ec-${bsf}_${year}${month}_pl850-pp-${harbor}-{}.grib || echo "NOT remap pl - no input or already produced"
...
[ -s ens/ec-${bsf}_$year${month}_mx2t24-unbound-24h-$abr-50-fix.grib ] && ! [ -s ens/ec-${bsf}_${year}${month}_mx2t24-unbound-${harbor}-50.grib ] && \
seq 0 50 | parallel cdo -b P8 -O --eccodes sellonlatbox,$bbox ens/ec-${bsf}_$year${month}_mx2t24-unbound-24h-$abr-{}-fix.grib ens/ec-${bsf}_${year}${month}_mx2t24-unbound-${harbor}-{}.grib || echo "NOT remap mx2t24 - no input or already produced"

# bias-adjusted mn2t24
[ -s ens/ec-${bsf}_$year${month}_mn2t24-unbound-24h-$abr-50-fix.grib ] && ! [ -s ens/ec-${bsf}_${year}${month}_mn2t24-unbound-${harbor}-50.grib ] && \
seq 0 50 | parallel cdo -b P8 -O --eccodes sellonlatbox,$bbox ens/ec-${bsf}_$year${month}_mn2t24-unbound-24h-$abr-{}-fix.grib ens/ec-${bsf}_${year}${month}_mn2t24-unbound-${harbor}-{}.grib || echo "NOT remap mn2t24 - no input or already produced"

# Instantaneous (10fg,tcc) at 00 UTC to ERA5 grid, and fitting grid points
[ -s ens/ec-sf_${year}${month}_all-24h-eu-50.grib ] && ! [ -s ens/ec-${bsf}_${year}${month}_inst-${harbor}-50.grib ] && \
seq 0 50 | parallel cdo -b P8 -O --eccodes sellonlatbox,$bbox -remap,$grid-$abr-grid,ec-sf-$grid-$abr-weights.nc -selname,10fg,tcc ens/ec-sf_${year}${month}_all-24h-eu-{}.grib ens/ec-${bsf}_${year}${month}_inst-${harbor}-{}.grib || echo "NOT remap sl - no input or already produced"
...
[ -s ens/ec-${bsf}_$year${month}_disacc-24h-$abr-50-fix.grib ] && ! [ -s ens/ec-${bsf}_${year}${month}_dailysums-unbound-${harbor}-50.grib ] && \
seq 0 50 | parallel cdo -b P8 -O --eccodes sellonlatbox,$bbox ens/ec-${bsf}_$year${month}_disacc-24h-$abr-{}-fix.grib ens/ec-${bsf}_${year}${month}_dailysums-unbound-${harbor}-{}.grib || echo "NOT unbound disacc - no input or already produced"
...
input2=ens/ec-${bsf}_${year}${month}_inst-${harbor}-{}.grib
input3=ens/ec-${bsf}_${year}${month}_dailysums-${harbor}-{}.grib
...
input8=ens/ec-${bsf}_${year}${month}_bound-${harbor}-{}.grib
...
#[ -s ens/ec-sf_${year}${month}_pl850-pp-${harbor}-50.grib ] && [ -s ens/ec-${bsf}_${year}${month}_inst-${harbor}-50.grib ] && [ -s ens/ec-${bsf}_${year}${month}_dailysums-${harbor}-50.grib ] && ! [ -s OCEANIDS/ens/ECXSF_${year}${month}_${predictand}_${harbor}-50.csv ] && \
    seq 0 50 | parallel python /home/ubuntu/bin/xgb-predict-era5-oceanids.py $input1 $input2 $input3 $input4 $input5 $input6 $input7 $input8 {} $predictand $harbor $output || echo "NOT predicting - no input or already produced"
