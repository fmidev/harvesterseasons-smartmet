...
i1=sys.argv[1] # sfc
i2=sys.argv[2] # pl
i3=sys.argv[3] # disacc
i4=sys.argv[4] # era5 oro
#ensmem = sys.argv[4] # ensemble member
target = sys.argv[5] # target variable in model
output = sys.argv[6] # output nc file

# import model
mod_dir='/home/ubuntu/data/MLmodels/xgb-bias/' # saved mdl
modname=mod_dir + f'xgb-bias_era5_ecsf_2020-2024_{target}.json'
...
# disacc
disacc=xr.open_dataset(i3, engine='cfgrib',
    backend_kwargs=dict(time_dims=('valid_time','verifying_time'),indexpath=''))
...
oro = xr.open_dataset(i4, engine='cfgrib',
    backend_kwargs=dict(time_dims=('valid_time','verifying_time'),indexpath=''))
...
ecsf = xr.merge([sl,disacc,pl850, pl925, oro_expanded], compat='override')
