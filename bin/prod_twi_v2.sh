...
[ ! -f $dtm-slope.tif ] && /usr/bin/saga_cmd ta_morphometry 0 -ELEVATION $dtm-filled.sdat -SLOPE $dtm-slope -ASPECT $dtm-aspect -METHOD 7\
 ...
 && parallel -j 2 gdalinfo -stats -hist ::: $dtm-slope $dtm-aspect  && s3cmd -P -q sync $dtm-aspect.tif s3://copernicus/dtm/aspect/ 
 #&& rm $dtm-slope.[smp]* $dtm-aspect.[smp]* && rm $dtm-aspect.tif
# Calculate catchments with FD8 multiple flow method Freeman 1991 and convergence 1.0 
[ ! -f $dtm-flow.tif ] && /usr/bin/saga_cmd ta_hydrology 0 -ELEVATION $dtm-filled.sdat -FLOW $dtm-flow -CONVERGENCE 1.0\
 && gdal_translate -of COG -co COMPRESS=DEFLATE -co BIGTIFF=IF_SAFER -co PREDICTOR=3 $dtm-flow.sdat $dtm-flow.tif
# && rm $dtm-flow.[smp]* && gdalinfo -stats -hist $dtm-flow.tif
# Calculate specific catchment and flow width Quinn et al 1991
[ ! -f $dtm-scarea.tif ] && /usr/bin/saga_cmd ta_hydrology 19 -DEM $dtm-filled.sdat -TCA $dtm-flow.sdat -WIDTH $dtm-flow-width -SCA $dtm-scarea -METHOD 1\
