...
[ ! -f $dtm-burn.tif ] && gdal_calc.py --quiet -A "$dtm_in" -B "$dtm-wbm.tif" --outfile="$dtm-burn-tmp.tif" --calc='A*(B<1)' --type Float32 --overwrite --NoDataValue=0 --co='COMPRESS=DEFLATE' --co='PREDICTOR=3' --co='BIGTIFF=IF_SAFER' --co='TILED=YES'\
 && gdal_translate -q -of COG -co COMPRESS=DEFLATE -co BIGTIFF=IF_SAFER -co PREDICTOR=3 $dtm-burn-tmp.tif $dtm-burn.tif && rm $dtm-burn-tmp.tif \
...
[ ! -f $dtm-filled.tif ] && /usr/bin/saga_cmd ta_preprocessor 5 -ELEV $dtm-burn.tif -FILLED $dtm-filled -MINSLOPE 0.01\
 && gdal_translate -q -of COG -co COMPRESS=DEFLATE -co BIGTIFF=IF_SAFER -co PREDICTOR=YES $dtm-filled.sdat $dtm-filled.tif\
 && rm $dtm-filled.[smp]*\
...
[ ! -f $dtm-slope.tif ] && /usr/bin/saga_cmd ta_morphometry 0 -ELEVATION $dtm-filled.tif -SLOPE $dtm-slope -ASPECT $dtm-aspect -METHOD 7\
 && parallel -j 2 gdal_translate -q -of COG -co COMPRESS=DEFLATE -co BIGTIFF=IF_SAFER -co PREDICTOR=3 -a_nodata -99999 {} {.}.tif ::: $dtm-slope.sdat $dtm-aspect.sdat\
...
[ ! -f $dtm-flow.tif ] && /usr/bin/saga_cmd ta_hydrology 0 -ELEVATION $dtm-filled.tif -FLOW $dtm-flow -CONVERGENCE 1.0\
 && gdal_translate -q -of COG -co COMPRESS=DEFLATE -co BIGTIFF=IF_SAFER -co PREDICTOR=3 -a_nodata -99999 $dtm-flow.sdat $dtm-flow.tif\
 && rm $dtm-flow.[smp]*
...
[ ! -f $dtm-scarea.tif ] && /usr/bin/saga_cmd ta_hydrology 19 -DEM $dtm-filled.tif -TCA $dtm-flow.tif -WIDTH $dtm-flow-width -SCA $dtm-scarea\
 && parallel -j 2 gdal_translate -q -of COG -co COMPRESS=DEFLATE -co BIGTIFF=IF_SAFER -co PREDICTOR=3 -a_nodata -99999 {} {.}.tif ::: $dtm-scarea.sdat $dtm-flow-width.sdat\
...
[ ! -f $dtm-twi.tif ] && /usr/bin/saga_cmd ta_hydrology 20 -SLOPE $dtm-slope.tif -AREA $dtm-scarea.tif -TWI $dtm-twi\
 && gdal_translate -q -of COG -co COMPRESS=DEFLATE -co BIGTIFF=IF_SAFER -co PREDICTOR=3 -a_nodata -99999 $dtm-twi.sdat $dtm-twi.tif\
 && rm $dtm-twi.[smp]*
