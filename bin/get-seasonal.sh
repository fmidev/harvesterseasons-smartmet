...
seq 0 50 | parallel -j 16 --compress --tmpdir tmp/ -q cdo --eccodes ymonmul \
    -remapbil,era5l-eu-grid -aexpr,'ws=sqrt(10u^2+10v^2);' -selvar,10u,10v ens/ec-sf_$year${month}_all-24h-eu-{}.grib \
    -aexpr,'10u=ws;10v=ws;' -selvar,ws era5l/era5l-ecsf_2000-2019_bound-filled_bias+varc.grib \
...
seq 0 50 | parallel -j 16 --compress --tmpdir tmp/ "cdo --eccodes mergetime -seltimestep,1 -selvar,e,tp ens/ec-sf_$year${month}_all-24h-eu-{}.grib \
    ...
    cdo --eccodes ymonmul -remapbil,era5l-eu-grid disacc-tmp-{}.grib\
     -selvar,e,tp era5l/era5l-ecsf_2000-2019_bound-filled_bias+varc.grib \
...
seq 0 50 | parallel -j 16 grib_set -r -s centre=98,setLocalDefinition=1,localDefinitionNumber=15,totalNumber=51,number={} ens/ec-bsf_$year${month}_acc-24h-eu-{}.grib \
