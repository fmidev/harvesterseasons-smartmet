...
# ECXSF 
sf_control='SWI2:ECXSF:5062:1:0:1:0'#:ensnro ERA5L control grid
sf='SWI2:ECXSF:5062:1:0:3'#:ensnro ERA5L grid
name='ecxsf'
...
#sf='SWI2:ECXSF:5022:1:0:0'#:ensnro
#name='ecbsf'
name='ecxsf'
ensdict={
...
#swi='SWI2:SWI:5022:1:0:0' # swi2 era5l grid
...
for i in range(len(lats)):
    ...
    start='20250302T000000Z'
    origintime='20250301T000000Z'
    query='http://'+source+'/timeseries?latlon='+lat+','+lon+'&param=utctime,'
    for par in ensdict.values():
        query+=par+','
    query=query[0:-1]
    query+='&starttime='+start+'&hour=0&timesteps=215&format=json&precision=full&tz=utc&timeformat=sql&origintime='+origintime
    print(query)
    response=requests.get(url=query)
    df=pd.DataFrame(json.loads(response.content))
    df.columns=['utctime']+list(ensdict.keys()) # change headers to params.keys
    ...
    # Fix: Merge using indexes instead of columns
    df3 = pd.merge(df, df2, left_index=True, right_index=True)
    print(df3)
    # No need to set index again as it's already set from the merge
    # df3=df3.set_index('utctime')  <- This line causes the error, remove it
    
    df_apu = df3.dropna(axis=1, how='all').dropna(axis=0, how='any')
    print(df_apu)
    if df_apu.empty:
        print('empty df')
        continue
        ...
        #df3=df3.dropna()
        mytitle=f'{start[:8]}, {country}, {pointid}, {lat}, {lon}'
        plot = df3.plot(figsize=(15,10),color=colorsdict, title=mytitle)
        fig = plot.get_figure()
        plt.legend(bbox_to_anchor=(1.0, 1.0))
        fig.savefig(f'/home/ubuntu/data/mlbias/figures/swi2/SWI2-figs_{start[:8]}_{name}-vsw2_{pointid}_{country}.png')
        #df.plot()
